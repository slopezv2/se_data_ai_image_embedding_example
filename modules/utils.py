...
def display_image_grid(
    ...
    """
    Displays a collection of images in a grid. The images can be loaded from
    a local directory or downloaded from an Azure Blob Storage container.
    ...
    :param container_client: A `ContainerClient` object (optional, for blob storage).
    :param images_dir: Local directory path containing images (optional, defaults to 'semart_dataset/Images').
    """
    num_images = len(image_names)

    # Load images from local directory or blob storage
    if images_dir or container_client is None:
        # Use local images
        import os
        if images_dir is None:
            images_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'semart_dataset', 'Images')
        
        images_stream = [
            os.path.join(images_dir, image_filename)
            for image_filename in image_names
        ]
        images = [Image.open(img_path) for img_path in images_stream]
    else:
        ...
        images = [Image.open(img_stream) for img_stream in images_stream]
